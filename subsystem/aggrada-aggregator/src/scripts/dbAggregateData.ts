...
import { db } from '../db';
import { AggConfig, getAggParams } from './getAggParams';
...
export const dbAggregateData = async ({
  ...
  const aggParams = await getAggParams(aggConfig).catch((error) => {
    console.error('Error retrieving aggregation parameters:', error);
    return null;
  });

  if (!aggParams) {
    console.error(
      'Error: Could not retrieve aggregation parameters. ',
      aggConfig
    );
    return;
  }
  console.log(
    ...
    aggParams.subdivisions.length,
    'subdivisions and',
    aggParams.temporalRanges.length,
    'temporal ranges.'
  );

  const createdAt = new Date();

  // Process subdivisions in batches of 5 in parallel
  for (let i = 0; i < aggParams.subdivisions.length; i += 3) {
    const subdivisionBatch = aggParams.subdivisions.slice(i, i + 3);

    await Promise.all(
      subdivisionBatch.map(async (subdivision) => {
        for (const temporalRange of aggParams.temporalRanges) {
          console.log(
            new Date().toISOString(),
            'Aggregating data for:',
            subdivision.geo_code,
            'from',
            temporalRange.start,
            'to',
            temporalRange.end
          ...
          if (!subdivisionGeometry?.raw_geometry) {
            console.error(
              'Error: Could not find raw_geometry for subdivision:',
              subdivision.geo_code
            );
          }

          const aggregatedDataKeys = {
            log_id: aggConfig.logId,
            spatial_id: subdivision.id as number,
            spatial_geo_code: subdivision.geo_code,
          ...
          try {
            await db.sequelize.query(
              ...
              WHERE
                ...
                AND o.core_file = ANY(ARRAY[:coreFiles])
              GROUP BY 
                k.key
              ON CONFLICT (
                spatial_id,
              ...
              `,
              {
                replacements: {
                  ...
                  subdivisionGeometry: JSON.stringify(
                    subdivisionGeometry.raw_geometry
                  ),
                  created_at: createdAt,
                  coreFiles: aggConfig.coreFiles,
                },
              }
            );
          } catch (error) {
