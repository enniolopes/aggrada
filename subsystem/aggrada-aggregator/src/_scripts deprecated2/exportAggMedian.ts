...
// export const exportAggregatedDataWithMedian = async ({
//   outputFilePath,
//   selectedDataKeys = [],
// }: {
...
//       dataKeys = (dataKeysResult as any[]).map((row) => {
//         return row.data_key;
//       });
//     }
...
//     // Construir a query dinâmica para calcular a mediana
...
//               ROW_NUMBER() OVER (ORDER BY value::numeric) AS row_num,
...
//               )
//             ) AS value
//             WHERE value ~ '^\\d+(\\.\\d+)?$' -- Garante que seja numérico
//           ) ranked_values
...
//     rawClient = await db.sequelize.connectionManager.getConnection({
//       type: 'read',
//     });
